...
return [
    ...
    'backup' => [
        ...
        'source' => [
            'files' => [
                /*
                 * SENGAJA DIPERSEMPIT dari default paket (base_path(), yaitu
                 * SELURUH direktori aplikasi) jadi cuma storage/app - kode
                 * sumber sudah aman di git (tidak perlu dibackup ulang), dan
                 * base_path() ikut menyeret .env (kredensial DB/DOKU/SMTP/
                 * Anthropic) ke dalam arsip backup, yang berisiko kalau
                 * BACKUP_DESTINATION_DISK diarahkan ke storage cloud dengan
                 * kontrol akses lebih longgar dari repo git. storage/app
                 * mencakup upload rapid-tier lama (lihat guratan-api/
                 * CLAUDE.md "Open security findings") - satu-satunya data
                 * pengguna yang genuinely tidak ada salinannya di database.
                 */
                'include' => [
                    storage_path('app'),
                ],
        ...
        'destination' => [
            ...
            /*
             * The disk names on which the backups will be stored. Default
             * 'local' disk - fine for a single-server dev/staging setup,
             * but a backup stored on the SAME disk as the data it's backing
             * up is not a real disaster-recovery plan. Set
             * BACKUP_DESTINATION_DISK to an S3-compatible disk (already
             * configurable via AWS_* env vars, see config/filesystems.php)
             * before relying on this in production - lihat DEPLOYMENT.md
             * "Backup database".
             */
            'disks' => [
    ...
    'notifications' => [
        /*
         * Sengaja HANYA notifikasi kegagalan/tidak-sehat yang aktif - kirim
         * email "sukses" tiap hari (default paket) cuma jadi noise yang
         * lama-lama diabaikan/di-filter, sementara justru kegagalan yang
         * WAJIB sampai ke seseorang. Nyalakan lagi Backup/CleanupWasSuccessful
         * kalau tim benar-benar mau konfirmasi harian eksplisit.
         */
        'notifications' => [
    ...
    'monitor_backups' => [
        [
            'name' => env('APP_NAME', 'laravel-backup'),
            'disks' => [env('BACKUP_DESTINATION_DISK', 'local')],
            'health_checks' => [
