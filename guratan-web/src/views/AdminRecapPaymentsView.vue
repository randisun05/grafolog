...
async function load(page = 1) {
  ...
  loadError.value = ''
  try {
    const { data } = await api.get('/admin/recap/payments', { params: { ...currentFilters(), page } })
    payments.value = data.data
    meta.value = { currentPage: data.current_page, lastPage: data.last_page, total: data.total }
  } catch (e) {
    loadError.value = e.response?.data?.message ?? 'Gagal memuat rekap pembelian laporan.'
...
async function exportCsv() {
  ...
  try {
    const response = await api.get('/admin/recap/payments/export', {
      params: currentFilters(),
      responseType: 'blob',
    })
    downloadBlob(response.data, `rekap-pembelian-laporan-${new Date().toISOString().slice(0, 10)}.csv`)
  } catch {
...
<template>
  <div class="admin-recap">
    <h1>Rekap Pembelian Laporan</h1>
    <p class="admin-recap__note">
      Transaksi pembelian laporan Comprehensive/Master, bisa difilter dan diekspor ke CSV.
    </p>
    ...
    <p v-else-if="payments.length === 0" class="admin-recap__empty">Tidak ada transaksi yang cocok.</p>
